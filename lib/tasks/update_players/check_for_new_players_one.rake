namespace :check_for_new_players_one do
	desc "Check for new players in Premier League, Serie A (Brazil), Primeira Liga"
	task setup: :environment do
		include PlayerSeasonCreatorHelper
		counter = ApiCallCount.first_or_create(count: 0)
		counter.update(count: 0)

		leagues = League.not_hidden.where(id: [1, 2, 3])
